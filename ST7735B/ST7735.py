...
class TFT(object) :
  ...
  def __init__( self, spi, aDC, aReset, aCS, screenW, screenH) :
    """aLoc SPI pin location is either 1 for 'X' or 2 for 'Y'.
       aDC is the DC pin and aReset is the reset pin."""
    self._size = (screenW, screenH)
...
#   @micropython.native
  ...
  def initg( self ) :
    ...
    self.windowLocData[1] = 0x00                #Start at row/column 1.
  ...
  def init80x160( self ) :
    # Initialize a 160x80 0.9 inch version.'''
      self._reset()
    ...
    # Rcmd2green160x80
      self._writecommand(TFT.CASET)                #Column address set.
      self.windowLocData[0] = 0x00
      self.windowLocData[1] = 24
      self.windowLocData[2] = 0x00
      self.windowLocData[3] = 80+24 - 1
      self._writedata(self.windowLocData)

      self._writecommand(TFT.RASET)                #Row address set.
      self.windowLocData[1] = 0
      self.windowLocData[3] = 160 - 1
      self._writedata(self.windowLocData)
...
'''
  def initb2( self, screenW, screenH )
